...
#考勤会影响职工工资
print("考勤成绩会影响职工工资，每1点考勤成绩可加50元，若缺勤超过5次，则扣500")
for k in range (times):
    print("第0"+ str(k) +"号职工工资增减")
    rs = '0' + str(k)
    sql8 = "SELECT * from staff_information WHERE 职工号 = '%s'" % rs
    cursor.execute(sql8)
    results = cursor.fetchall()
    ...
    if flag == 1:
        ...
        print(str(50*int(grade)))



#动态sql
print("请输入是否要突击考察一位员工，如果是，请输入其职工号，如果否，请输入N")
rs = input()

if (rs != 'N'):
    sql10 = "UPDATE staff_information SET 考勤成绩 = 考勤成绩 + 1 WHERE 职工号 = '%s'" % rs
    # 该考勤成员未到
    print("请输入该考勤成员是否到达，到达请输入1，未到请输入0")
    m = int(input())
    if (m == 0):
        # 读取被考勤人员的信息
        cursor.execute(sql4)
    ...
    #修改其信息
    sql9 = "UPDATE staff_information SET 职工部门 = '行政部' WHERE 职工号 = '%s'" % rs
    cursor.execute(sql9)
    db.commit()
